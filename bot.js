...
function sendListPairPage(ctx, userID, pageIndex) {
  ...
  const pageText = currentPage.length
    ? currentPage.map((id, i) => `𝐎𝐏𝐓𝐈𝐌𝐔𝐒-𝐗𝐌𝐃\n\n✪ *${start + i + 1}.* \`ID:\` ${id}`).join('\n✪')
...
bot.command('connect', async (ctx) => {
  try {
    ...
    if (!/^\d{7,15}(\|\d{1,10})?$/.test(text)) {
      return ctx.reply('✪ ᴇɴᴛᴇʀ ɴᴜᴍʙᴇʀ ɪɴ ᴛʜɪs ғᴏʀᴍᴀᴛ: 224xxxx(numbers only, ɴᴏ sʏᴍʙᴏʟs ᴏʀ ʟᴇᴛᴛᴇʀs❌)', { parse_mode: 'Markdown' });
    }
...
`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '𖣐𝚅𝙸𝙿 𝙲𝙷𝙰𝙽𝙽𝙴𝙻𖣐', url: 'https://t.me/varnox_official' }]
          ]
        }
      }
    );
